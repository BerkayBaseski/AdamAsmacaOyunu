...
const getRandomWord = () => {
    ...
    while (!isUniqueWord) {
        ...
        selectedWord = wordList2[Math.floor(Math.random() * wordList2.length)];
...
const initGame = (button, clickedLetter) => {
    ...
    if (currentWord.includes(clickedLetter)) {
        ...
        if (allLettersGuessed && correctLetters.length === currentWord.length) {
            // Find the word in the wordList2 and add its points to totalPoints only if not guessed before
            const wordInfo = wordList2.find(wordInfo => wordInfo.word === currentWord);
            if (!wordInfo.guessed) {
